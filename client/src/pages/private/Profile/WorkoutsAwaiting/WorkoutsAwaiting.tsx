...
import styles from '../WokoutHistory/workouts_history.module.css';
...
export function WorkoutsAwaiting() {
    ...
    useEffect(() => {
        Api.get(`/api/get/workouts/user?u=${session.id}&sort=not`, {
    ...
    return (
        <ProfilePanel>
            {isLoading && <Spinner />}
            <LabelPanel link={`/${session.id}/profile`}>Предстоящие тренеровки</LabelPanel>
            {workoutsData.length > 0 ? (
                ...
                <h3 className={cxPanel('page_hint')}>Предстоящих тренеровок не найдено</h3>
            )}
        </ProfilePanel>
    );
}
